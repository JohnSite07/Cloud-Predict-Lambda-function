...
def lambda_handler(event, context):
    ...
    table = dynamodb.Table('jl-portfolio-data-table')
    ...
    # --- Insert your code here ---
    sentiment = find_max_sentiment(comprehend.batch_detect_sentiment(
                        TextList=[
                            enquiry_text 
                                ],
                        LanguageCode = 'en'
    ...
    # --- Insert your code here ---
    name = dec_dict['name']
    critical_phrase_list = ['github', 'git', 'Git','GitHub', 'projects', 'portfolio',
                             'Portfolio', 'C.V', 'resume', 'Curriculum Vitae','Resume', 'CV']
    list_of_extracted_phrases = list(pd.DataFrame(comprehend.batch_detect_key_phrases(
                    TextList=[enquiry_text],
                    LanguageCode = 'en')["ResultList"][0]['KeyPhrases'])['Text'])
    def get_sentiment(sentence):
        sentiment = comprehend.batch_detect_sentiment(
                    TextList=[sentence],
                    LanguageCode = 'en')
        return sentiment['ResultList'][0]
    AWS_Comprehend_Sentiment_Dump = get_sentiment(enquiry_text)
